...
export default [
    ...
    //安装计划
    {
        ...
        response: () => {
            ...
            const a = Mock.mock({
                ["category|" + num]: ["@city()"],
                ["cpu_data|" + num]: ["@integer(10, 100)"],
            })
            let gpu_data = [], memory_data = [];
            for (let index = 0; index < num; index++) {
                let lineNum = Mock.mock('@integer(0, 100)') + a.cpu_data[index]
                gpu_data.push(lineNum)
                let rate = a.cpu_data[index] / lineNum;
                memory_data.push((rate * 100).toFixed(0))
            }
            a.gpu_data = gpu_data
            a.memory_data = memory_data
            return {
                success: true,
                data: a
            }

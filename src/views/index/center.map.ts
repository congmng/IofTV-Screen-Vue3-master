...
export const optionHandle = (regionCode: string,
    ...
    return {
        ...
        visualMap: {
            ...
            bottom: 20,
            pieces: [
                { gte: 51, label: "51台以上" }, // 不指定 max，表示 max 为无限大（Infinity）。
                { gte: 21, lte: 50, label: "21-50台" },
                { gte: 11, lte: 20, label: "11-20台" },
                { gte: 6, lte: 10, label: "6-10台" },
                { gte: 2, lte: 5, label: "2-5台" },
                { lte: 1, label: "1台" }, // 不指定 min，表示 min 为无限大（-Infinity）。
            ],
            inRange: {
                // 渐变颜色，从小到大
